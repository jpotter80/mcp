...
1. **CRITICAL - Testing Branch Workflow**: 
...
3. **Dependency Management**: Use `pixi add` for new packages; check `pixi.toml` first (root & server-specific)
4. **Git Strategy**: Checkout restructure branches locally, merge to main — no remote pushes until complete
5. **Code Output**: Avoid unnecessary summaries; use tools directly for changes
6. **Key Dependencies**: `python-frontmatter` already in root `pixi.toml`
7. **Superfluous Summaries**: Do NOT create overview/summary documents unless explicitly requested
