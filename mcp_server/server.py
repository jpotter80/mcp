"""
MCP server exposing Mojo docs hybrid search via resources and tools.

Requires: pip install "mcp[cli]"
Run (dev inspector):
    mcp dev mcp_server/server.py

This server reuses the HybridSearcher from search.py and holds a shared
read-only DuckDB connection for fast queries.
"""
...
# Ensure project root is on sys.path so we can import search.py
_ROOT = Path(__file__).resolve().parent.parent
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))

# Reuse our search implementation via dynamic import (avoids E402 linter issues)
_search_mod = importlib.import_module("search")
HybridSearcher = getattr(_search_mod, "HybridSearcher")
TOP_K = getattr(_search_mod, "TOP_K")
...
class AppState:
    def __init__(self, searcher: Any, max_proc: Optional[subprocess.Popen] = None) -> None:  # type: ignore[name-defined]
...
def _ensure_max_running(base_url: str, model_name: str, auto_start: bool = True, wait_s: float = 20.0) -> Optional[subprocess.Popen]:  # type: ignore[name-defined]
...
async def app_lifespan(server: FastMCP) -> AsyncIterator[AppState]:
    ...
    base_url = os.getenv("MAX_SERVER_URL", getattr(_search_mod, "MAX_SERVER_URL", "http://localhost:8000/v1"))
    model_name = os.getenv("EMBED_MODEL_NAME", getattr(_search_mod, "MODEL_NAME", "sentence-transformers/all-mpnet-base-v2"))
    auto_start = os.getenv("AUTO_START_MAX", "1") not in ("0", "false", "False")
    max_proc = _ensure_max_running(base_url, model_name, auto_start=auto_start)

    searcher = HybridSearcher()  # opens read-only DuckDB, loads vss+fts
    try:
...
@mcp.tool()
def search(query: str, k: int = TOP_K, ctx: Optional[Context] = None) -> List[SearchResult]:
    """Hybrid search over Mojo docs. Returns top-k results with snippets.
...
if __name__ == "__main__":
    # Allow direct execution for convenience (e.g., python mcp_server/server.py)
    mcp.run()
