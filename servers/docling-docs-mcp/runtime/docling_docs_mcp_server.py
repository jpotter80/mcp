...
Usage patterns:
- Replace placeholders like docling-docs-mcp with actual values during scaffolding
...
async def app_lifespan(server: FastMCP) -> AsyncIterator[AppState]:
    ...
    raw_db_path = db_config.get("path", os.getenv("DOCLING_DOCS_MCP_DB_PATH", "docling_docs_mcp.db"))
    ...
    table_name = db_config.get("table_name", os.getenv("DOCLING_DOCS_MCP_TABLE_NAME", "docling_docs_indexed"))
...
mcp = FastMCP("Docling Docs", lifespan=app_lifespan)
...
@mcp.resource("docling-docs-mcp://search/{q}")
...
@mcp.resource("docling-docs-mcp://chunk/{chunk_id}")
