...
Usage patterns:
- Replace placeholders like mcp-docs-mcp with actual values during scaffolding
...
async def app_lifespan(server: FastMCP) -> AsyncIterator[AppState]:
    ...
    raw_db_path = db_config.get("path", os.getenv("MCP_DOCS_MCP_DB_PATH", "mcp_docs_mcp.db"))
    ...
    table_name = db_config.get("table_name", os.getenv("MCP_DOCS_MCP_TABLE_NAME", "mcp_docs_indexed"))
...
mcp = FastMCP("Mcp Docs", lifespan=app_lifespan)
